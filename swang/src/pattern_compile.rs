//! The pattern compiler: kernel literal → `griff-pattern` expansion →
//! `griff-swang` lowering → an explicit palette for the shared generation
//! compiler, plus the versioned expansion artifact (spec §1.14).
//!
//! This is the one place the pattern pipeline lives. Both the Phase-2
//! `--rhythm-*` transport (`griff generate`) and the Phase-3 evaluator (the
//! `eval` module) drive it, so neither can drift. The pipeline fails as
//! typed [`PatternFlaw`] data (spec §1.5: the core emits pure data); the
//! transport renders it as [`PatternDiagnostic`]s — a stable `SWG____` code,
//! the offending flag, and a message — while the Swang frontends render the
//! same flaws against their program spans.
//!
//! The value enums here are plain (no argument-parsing derive): the CLI owns
//! its own clap `ValueEnum`s and converts into these, keeping `griff-swang`
...
pub fn compile_pattern_flaws(
    ...
    let tail = match args.tail {
        TailChoice::Reject => crate::TailPolicy::Reject,
        TailChoice::RestPad => crate::TailPolicy::RestPad,
    };
    let templates =
        crate::map_rhythm(&sequence, bar_duration, unit, tail).map_err(PatternFlaw::Lower)?;
...
fn map_lower_error(e: crate::LowerError) -> PatternDiagnostic {
    match e {
        crate::LowerError::UnitDoesNotDivideBar { bar_duration, unit } => PatternDiagnostic {
            code: "SWG0301",
            flag: "--rhythm-unit",
            message: format!(
        ...
        crate::LowerError::ZeroUnit => PatternDiagnostic {
            code: "SWG0301",
            flag: "--rhythm-unit",
