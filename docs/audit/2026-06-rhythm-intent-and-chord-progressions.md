...
| Custom grammar / command language (`set meter 4/4`, `for bars 1..4: …`) | Rejected. The same semantics decompose 1:1 into the typed request specs of this note (§2.1 rhythm + §2.2 progression + §2.4 performance + existing `GenerationConstraints`), so a grammar adds syntax, not capability. A plain serde data file (YAML/TOML) over those structs delivers every real benefit of "spec as text" — shareable presets, diffs, batch/CLI, fixtures — with no parser, no error-position machinery, no dialect drift; and by our own rules a new textual input format is an external format adapter (ADR-0010: fuzz target mandatory). The imperative `set`/`disable` form additionally smuggles ordering/state into a declarative spec. Revisit only if hand-written spec files become frequent enough that verbosity demonstrably hurts — then sugar may *compile into the same structs*, never become a second semantic path. |
| Configurable `symbol_map` for the pattern alphabet | Rejected: a fixed alphabet (`x` onset, `.` sustain, `-` rest, `m` mute) keeps every pattern file readable by everyone; per-file symbol maps make each file its own dialect. |
